...
data<-read.csv("Isla500_As_Meth.csv")
colnames(data)[1]<-"ID"
data[1,1]<-1
rownames(data)<-data[,1]
data<-data[,-1]

G1<-Mclust(data[,c(1,6)],G=3,modelNames="VVV")

G2<-Mclust(data[,c(2,7)],G=7,modelNames="VVV")

G3<-Mclust(data[,c(3,8)],G=7,modelNames="VVV")

G4<-Mclust(data[,c(4,9)],G=7,modelNames="VVV")

G5<-Mclust(data[,c(5,10)],G=7,modelNames="VVV")
...
M=kde2d(bmean,gmean,n=100)
#col5<- matlab.like(length(M$z))[rank(M$z)]
persp3d(M$x,M$y,M$z,xlab="Methylation",ylab="Expression",zlab="Density", box=FALSE)
...
data<-cbind(ID=rownames(data),data)
head(data)
data.df=data.frame(data[,1],time=t1,X=data[,2],Y=data[,7])
data.df=data.frame(data[,1],time="t1",X=data[,2],Y=data[,7])
head(data.df)
data.df=data.frame(IDdata[,1],time="t1",X=data[,2],Y=data[,7])
data.df=data.frame(ID=data[,1],time="t1",X=data[,2],Y=data[,7])
data.df=rbind(data.df,data.frame(ID=data[,1],time="t2",X=data[,3],Y=data[,8])
)
data.df=rbind(data.df,data.frame(ID=data[,1],time="t3",X=data[,4],Y=data[,9]))
data.df=rbind(data.df,data.frame(ID=data[,1],time="t4",X=data[,5],Y=data[,10]))
data.df=rbind(data.df,data.frame(ID=data[,1],time="t5",X=data[,6],Y=data[,11]))
dim(data)

xyplot(Y~X,data=data.df,group=ID,type="b")


names2<-names(G1$classification[G1$classification==2])
names3<-names(G1$classification[G1$classification==3])
names4<-names(G1$classification[G1$classification==4])
names5<-names(G1$classification[G1$classification==5])
names6<-names(G1$classification[G1$classification==6])
names7<-names(G1$classification[G1$classification==7])
